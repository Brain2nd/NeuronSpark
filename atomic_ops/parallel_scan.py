...
if _HAS_TRITON:
    ...
    # ============================================================
    # Fused PLIF row-param + SEW + V_post recomputation (memory-optimal)
    # Forward: only stores spike (not V_post), spike_sew = spike + spike_in
    # Backward: recomputes V_post from (u, beta, v_th, spike, v_init)
    # Saves ~50% autograd memory vs _TritonPLIFRowParamSEWForward
    # ============================================================

    @triton.jit
    def _fused_plif_fwd_rowparam_sew_noV_kernel(
        BETA_ROW_ptr, U_ptr, VTH_ROW_ptr, INIT_ptr,
        SPIKE_IN_ptr,
        SPIKE_ptr, SPIKE_SEW_ptr, V_LAST_ptr,
        K, num_cols,
        BLOCK: tl.constexpr,
    ):
        """Forward: stores spike, spike_sew, and v_last (final V_post). V_post recomputed in backward."""
        pid = tl.program_id(0)
        cols = pid * BLOCK + tl.arange(0, BLOCK)
        mask = cols < num_cols
        ...
        for k in range(K):
            ...
            # Store spike (for backward recomputation) and spike_sew (for return)
            tl.store(SPIKE_ptr + off, spike, mask=mask)
            tl.store(SPIKE_SEW_ptr + off, spike + spike_in, mask=mask)

        # Store final V_post for state update (not saved to autograd graph)
        tl.store(V_LAST_ptr + cols, v, mask=mask)

    @triton.jit
    def _fused_plif_bwd_rowparam_sew_recomp_kernel(
        BETA_ROW_ptr, VTH_ROW_ptr, INIT_ptr,
        U_ptr, SPIKE_ptr, SPIKE_IN_ptr,
        GRAD_SPIKE_SEW_ptr, GRAD_VPOST_ptr,
        GRAD_BETA_ROW_ptr, GRAD_U_ptr, GRAD_VTH_ROW_ptr, GRAD_INIT_ptr, GRAD_SPIKE_IN_ptr,
        VPOST_TEMP_ptr,
        K, num_cols, ALPHA,
        BLOCK: tl.constexpr,
    ):
        """Backward with V_post recomputation + SEW gradient propagation."""
        pid = tl.program_id(0)
        cols = pid * BLOCK + tl.arange(0, BLOCK)
        mask = cols < num_cols
        ...
        # Phase 1: Forward recomputation of V_post → temp buffer
        v = v_init_val
        for k in range(K):
            off = k * num_cols + cols
            u = tl.load(U_ptr + off, mask=mask, other=0.0).to(tl.float32)
        ...
        for k_rev in range(K):
            ...
            # grad_spike_sew flows to both spike and spike_in (straight-through)
            g_s = tl.load(GRAD_SPIKE_SEW_ptr + off, mask=mask, other=0.0).to(tl.float32)
            g_V = tl.load(GRAD_VPOST_ptr + off, mask=mask, other=0.0).to(tl.float32)

            # Store grad_spike_in = grad_spike_sew (straight-through for SEW add)
            tl.store(GRAD_SPIKE_IN_ptr + off, g_s, mask=mask)
    ...
    class _TritonPLIFRowParamSEWRecompute(torch.autograd.Function):
        """Fused Triton PLIF + SEW + V_post recomputation (memory-optimal).

        Same computation as _TritonPLIFRowParamSEWForward, but does NOT store
        V_post across forward→backward gap. Backward recomputes V_post from
        (u, beta, v_th, spike, v_init) — 16 steps of multiply-add in registers.

        Saves ~50% autograd memory (no V_post tensor in ctx.saved_tensors).
        Optimal for K≤32 where recomputation cost is negligible.
        """
        ...
        def forward(ctx, beta_row, u, v_th_row, v_init, spike_in_seq, alpha):
            ...
            spike = torch.empty_like(u_c)
            spike_sew = torch.empty_like(u_c)
            # v_last: (batch, D) - only the final V_post for state update
            v_last = torch.empty_like(v_init_c)

            BLOCK = _TritonPLIFRowParamSEWRecompute._BLOCK
            grid = ((num_cols + BLOCK - 1) // BLOCK,)

            _fused_plif_fwd_rowparam_sew_noV_kernel[grid](
                beta_row_c, u_c, v_th_row_c, v_init_c,
                spike_in_c,
                spike, spike_sew, v_last,
                K, num_cols,
                BLOCK=BLOCK,
            )

            # Memory-optimal: only save tensors needed for backward recomputation
            # No V_post! Saves ~50% autograd memory
            if any(ctx.needs_input_grad[:5]):
                ctx.save_for_backward(beta_row_c, v_th_row_c, v_init_c, spike, u_c, spike_in_c)
            ctx.K = K
            ctx.num_cols = num_cols
            ctx.alpha = alpha

            # Return spike_sew and v_last (for state update, not saved to autograd)
            # v_last is (batch, D), need to expand to (1, batch, D) for compatibility
            return spike_sew, v_last.unsqueeze(0)

        @staticmethod
        def backward(ctx, grad_spike_sew, _grad_V_post_unused):
            beta_row, v_th_row, v_init, spike, u, spike_in = ctx.saved_tensors
            K = ctx.K
            num_cols = ctx.num_cols
            alpha = ctx.alpha

            if grad_spike_sew is None:
                grad_spike_sew = torch.zeros_like(spike)

            grad_spike_sew_c = grad_spike_sew.contiguous()
            # V_post gradient is always zero (None output)
            ...
            grad_v_th_row = torch.empty_like(v_th_row)
            grad_v_init = torch.empty_like(v_init)
            grad_spike_in = torch.empty_like(spike_in)
            ...
            _fused_plif_bwd_rowparam_sew_recomp_kernel[grid](
                beta_row, v_th_row, v_init,
                u, spike, spike_in,
                grad_spike_sew_c, grad_V_post_c,
                grad_beta_row, grad_u, grad_v_th_row, grad_v_init, grad_spike_in,
                V_post_temp,
                K, num_cols, float(alpha),
                BLOCK=BLOCK,
            )
...
def plif_rowparam_forward_sew(
    ...
    v8.0 变更: 使用 V_post recomputation 版本，节省 ~50% autograd 内存。
    Forward 只保存 spike（不保存 V_post），backward 时重新计算 V_post。
    对于 K≤32 的短序列，重计算开销可忽略。
    ...
    Returns:
        ...
        V_post:    (K, *shape) — 发放后膜电位（None，使用 recompute 版本不返回 V_post）
    """
    if _HAS_TRITON and u.is_cuda:
        # 使用 V_post recomputation 版本，节省 ~50% autograd 内存
        spike_sew, V_post = _TritonPLIFRowParamSEWRecompute.apply(
