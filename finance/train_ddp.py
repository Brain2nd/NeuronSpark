"""
分布式金融模型训练脚本：Behavioral Cloning（DDP 多卡并行）

基于 finance/train.py 单卡脚本，使用 PyTorch DDP 实现数据并行。
训练逻辑、模型架构、超参数与单卡版完全一致。

用法：
    # 单机多卡
    torchrun --nproc_per_node=4 finance/train_ddp.py \
        --features_path data/finance/features.npy \
        --targets_path data/finance/targets.npy

    # 单卡也能跑
    torchrun --nproc_per_node=1 finance/train_ddp.py \
        --features_path data/finance/features.npy \
        --targets_path data/finance/targets.npy

    # 断续训练
    torchrun --nproc_per_node=4 finance/train_ddp.py \
        --resume checkpoints_finance/ckpt_step1000.pth \
        --features_path data/finance/features.npy \
        --targets_path data/finance/targets.npy
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SNN Finance Model Training (BC, DDP)")
    ...
    Logger(f"\n{'='*60}", rank)
    Logger(f"SNN Finance Model Training (BC, DDP, {world_size} GPUs)", rank)
    ...
    if is_main_process(rank):
        Logger(f"\nTraining finished. Total samples seen: {samples_seen:,}", rank)
