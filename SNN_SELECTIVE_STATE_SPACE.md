...
- SelectivePLIFNode, ParametricLIFNode: 神经元动力学
...
```
spike_seq ← encode(all_tokens)                   // (TK, batch, D)

for l = 1 to L:
  spike_seq ← SNNDecoderLayer_l.forward_parallel(spike_seq)
    // 内部: SNNBlock parallel + SNNFFN parallel
    // 并行深度: O(log(TK))
...
$$\text{logits}_n = \text{Embedding}^T \cdot \text{RMSNorm}\!\left(\text{decode\_proj}(\hat{y}_n)\right) \quad \forall n$$
...
6. 训练采用 IG-ZO（信息论引导的零阶优化，见第 9 节）：六个设计目标的诊断驱动定向扰动，Fisher 加权，多时间尺度学习率
