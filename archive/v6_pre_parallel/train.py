...
class SPSAOptimizer:
    """
    多扰动 SPSA (Simultaneous Perturbation Stochastic Approximation) 零阶优化器。

    M 个独立扰动方向取平均，梯度估计方差降 M 倍。
    使用种子重放法：评估阶段只存种子+标量，更新阶段重新生成 delta，
    避免 M 组 delta（每组 4.4GB）同时驻留内存。
    ...
    def generate_perturbation(self, seed=None) -> list[torch.Tensor]:
        """生成 Rademacher 随机扰动（±1）。可选种子用于重放。"""
        saved_state = None
        if seed is not None:
            device = self.params[0].device
            if device.type == 'cuda':
                saved_state = torch.cuda.get_rng_state(device)
        ...
        if saved_state is not None:
            if self.params[0].device.type == 'cuda':
                torch.cuda.set_rng_state(saved_state, self.params[0].device)
            else:
    ...
    def step(self, grad_scales_and_seeds: list[tuple[float, int]]):
        """多扰动 SPSA 梯度估计 + 参数更新。

        用种子重放 delta，逐个方向累加更新，内存开销 = 1 组 delta。

        Args:
            grad_scales_and_seeds: M 个 (grad_scale, seed) 对
        """
        self._update_schedule()
        M = len(grad_scales_and_seeds)

        with torch.no_grad():
            for grad_scale, seed in grad_scales_and_seeds:
                clipped = max(min(grad_scale, self.grad_clip), -self.grad_clip)
                deltas = self.generate_perturbation(seed=seed)
                for p, d in zip(self.params, deltas):
                    p.add_(d, alpha=-self.lr * clipped / M)
                del deltas
    ...
    def load_state_dict(self, state):
        ...
        self.num_perturbations = state.get('num_perturbations', self.num_perturbations)
        self._update_schedule()
...
def train_epoch(epoch, model, train_loader, optimizer, args, iter_per_epoch, tokens_seen):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        for i in range(optimizer.num_perturbations):
            # 生成种子并记录（更新阶段重放用）
            seed = torch.randint(0, 2**62, (1,)).item()
            seeds.append(seed)

            # 生成扰动
            deltas = optimizer.generate_perturbation(seed=seed)

            # θ + c·δ → L+
            optimizer.perturb(deltas, sign=+1.0)
            with torch.no_grad(), torch.cuda.amp.autocast(dtype=torch.bfloat16, enabled=args.use_bf16):
                out = model(X, Y)
                loss_plus = (torch.sum(out.last_loss * loss_mask_flat) / valid_tokens).item()

            # θ - 2c·δ → L-（从 +c 到 -c 需要 -2c）
            ...
            grad_scale = (loss_plus - loss_minus) / (2.0 * optimizer.c)
            grad_scales.append(grad_scale)
            loss_sum += (loss_plus + loss_minus) / 2.0

            del deltas, out

        # SPSA 更新（种子重放，逐方向累加）
        optimizer.step(list(zip(grad_scales, seeds)))
        ...
        # 记录
        batch_loss = loss_sum / optimizer.num_perturbations
        batch_ppl = math.exp(min(batch_loss, 20.0))  # clamp 防溢出
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_perturbations', type=int, default=1, help='每步扰动方向数（M），方差降 M 倍')
    parser.add_argument('--use_bf16', action='store_true', default=True, help='启用 bfloat16 autocast 加速前向')
    parser.add_argument('--no_bf16', action='store_true', help='禁用 bfloat16')
    ...
    Logger(f"  Perturb:     {args.perturbation_scale} × {args.num_perturbations} directions")
