"""
金融模型训练脚本：Behavioral Cloning / SFT（阶段 1-2）

训练 SNNFinanceModel 输出多维连续交易决策（position, leverage, stop_loss, take_profit）。
遵循与预训练脚本相同的基础设施（SNNAdamW + layer-wise LR 校准 + cosine 调度）。

三阶段统一训练管道:
  Stage 1 (BC):  Huber loss, targets from rule-based strategy（本脚本）
  Stage 2 (SFT): Huber loss, targets from backtested optimal（复用本脚本，换数据）
  Stage 3 (RL):  PPO/A2C, reward = PnL（需单独 RL 脚本）

所有阶段统一 I/O: (batch, seq_len, n_features) → (batch, seq_len, n_assets, 4)

用法:
    conda activate SNN

    python finance/train.py \
        --features_path data/finance/features.npy \
        --targets_path data/finance/targets.npy \
        --batch_size 16 --accumulation_steps 4

    # 断续训练
    python finance/train.py --resume checkpoints_finance/ckpt_step1000.pth
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SNN Finance Model Training (BC/SFT)")

    # 模型参数
    parser.add_argument('--n_features', type=int, default=987)
    parser.add_argument('--n_assets', type=int, default=1,
                        help='交易资产数（1=单品种，3=三交易对）')
    ...
    parser.add_argument('--max_leverage', type=float, default=10.0,
                        help='杠杆上限')
    parser.add_argument('--sl_min', type=float, default=0.005, help='止损下限 (0.5%)')
    parser.add_argument('--sl_max', type=float, default=0.10, help='止损上限 (10%)')
    parser.add_argument('--tp_min', type=float, default=0.01, help='止盈下限 (1%)')
    parser.add_argument('--tp_max', type=float, default=0.20, help='止盈上限 (20%)')
    parser.add_argument('--recon_weight', type=float, default=0.1,
                        help='重建辅助损失权重')

    # 训练参数
    parser.add_argument("--out_dir", type=str, default="checkpoints_finance")
    ...
    parser.add_argument("--features_path", type=str, required=True,
                        help='特征文件 (.npy, shape=(total_steps, n_features))')
    parser.add_argument("--targets_path", type=str, required=True,
                        help='目标文件 (.npy, shape=(total_steps, n_assets*4))')
    ...
    _wd_groups = {
        'input_proj': 0.1, 'decision_head': 0.1, 'decode': 0.1,
    ...
    Logger(f"\n{'='*60}")
    Logger(f"SNN Finance Model Training (BC/SFT)")
    ...
    Logger(f"  Ranges:      leverage [1, {args.max_leverage}], SL [{args.sl_min*100:.1f}%, {args.sl_max*100:.0f}%], TP [{args.tp_min*100:.0f}%, {args.tp_max*100:.0f}%]")
    ...
    Logger(f"  Recon weight: {args.recon_weight}")
    ...
    Logger(f"\nTraining finished. Total samples seen: {samples_seen:,}")
    if args.device != 'cpu':
