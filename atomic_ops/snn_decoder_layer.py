...
class SNNDecoderLayer(base.MemoryModule):
    ...
    def forward_parallel(self, spike_in_seq):
        """
        并行前向传播：全序列通过 SNNBlock + SNNFFN。

        Args:
            spike_in_seq: (TK, batch, D) — 全部 T×K 帧的输入 spike

        Returns:
            spike_out_seq: (TK, batch, D)
        """
        spike_mid = self.snn_block.forward_parallel(spike_in_seq)
        spike_out = self.snn_ffn.forward_parallel(spike_mid)
        return spike_out
